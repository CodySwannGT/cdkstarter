#!/usr/bin/env bash
#
# mysql-secret.sh — open a `mysql` shell against the Aurora cluster endpoint
...
# directly (which still accepts mysql_native_password).
#
# Starts an SSM tunnel to the cluster writer endpoint via the relay, fetches
# the master credentials from Secrets Manager, and drops you into the MySQL
# client. The tunnel is torn down automatically on exit.
#
# MySQL counterpart of psql-secret.sh — use this when the Aurora cluster runs
# the MySQL engine instead of Postgres.
#
# Usage:
#   scripts/mysql-secret.sh <dev|staging|production> [local-port] [-- mysql-args...]
#
# Examples:
#   scripts/mysql-secret.sh dev
#   scripts/mysql-secret.sh dev 13306
#   scripts/mysql-secret.sh dev -e "SELECT @@version;"
#
# AWS profile resolution: $AWS_PROFILE if set, else "$AWS_PROFILE_PREFIX-<env>"
# (AWS_PROFILE_PREFIX defaults to "your-project").
#
# Prefer mysql-iam.sh for routine work — tokens beat cleartext passwords.
...
case "$ENV_ARG" in
  ...
  *)
    echo "Usage: $0 <dev|staging|production> [local-port] [mysql-args...]" >&2
    exit 1
    ;;
esac
shift
...
for cmd in aws session-manager-plugin lsof jq; do
  if ! command -v "$cmd" >/dev/null 2>&1; then
    ...
    echo "  macOS: brew install awscli mysql-client jq && brew install --cask session-manager-plugin" >&2
    exit 1
  fi
done

# mysql-client is keg-only on macOS; fall back to brew --prefix.
MYSQL_BIN="$(command -v mysql || true)"
if [ -z "$MYSQL_BIN" ] && command -v brew >/dev/null 2>&1; then
  PFX="$(brew --prefix mysql-client 2>/dev/null || true)"
  [ -n "$PFX" ] && [ -x "$PFX/bin/mysql" ] && MYSQL_BIN="$PFX/bin/mysql"
fi
if [ -z "$MYSQL_BIN" ]; then
  echo "Error: mysql client not found. Install via: brew install mysql-client" >&2
  exit 1
fi
...
if [ -z "$REMOTE_PORT" ] || [ "$REMOTE_PORT" = "None" ]; then REMOTE_PORT=3306; fi
...
TUNNEL_LOG="$(mktemp -t mysql-secret-tunnel.XXXXXX)"
...
# Cache the RDS global CA bundle once (the cluster cert is signed by Amazon
# RDS's private CA, not in the system trust store).
...
# MYSQL_PWD keeps the password out of `ps`. VERIFY_CA validates the chain but
# skips the hostname check (we connect to 127.0.0.1, cert is for the cluster
# hostname). The cluster accepts mysql_native_password, so no cleartext plugin.
MYSQL_PWD="$DB_PASS" "$MYSQL_BIN" -h 127.0.0.1 -P "$LOCAL_PORT" -u "$DB_USER" \
  --ssl-mode=VERIFY_CA --ssl-ca="$RDS_CA_BUNDLE" \
