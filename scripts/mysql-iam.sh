#!/usr/bin/env bash
#
# mysql-iam.sh — open a `mysql` shell against the Aurora RDS Proxy using IAM
# authentication. Starts an SSM Session Manager tunnel to the relay,
# generates a short-lived IAM auth token, and drops you into the MySQL
# client. The tunnel is torn down automatically on exit.
#
# MySQL counterpart of psql-iam.sh — use this when the Aurora cluster runs
# the MySQL engine instead of Postgres.
#
# Usage:
#   scripts/mysql-iam.sh <dev|staging|production> [local-port] [-- mysql-args...]
#
# Examples:
#   scripts/mysql-iam.sh dev
#   scripts/mysql-iam.sh dev 13306
#   scripts/mysql-iam.sh dev -e "SELECT @@version;"
#   scripts/mysql-iam.sh dev 13306 -- --xml -e "SHOW DATABASES;"
#
...
TUNNEL_LOG="$(mktemp -t mysql-iam-tunnel.XXXXXX)"
...
# VERIFY_CA skips the hostname check (we connect to 127.0.0.1 but the cert
# is for the proxy's real hostname); the cert chain is still validated.
# --enable-cleartext-plugin is required for IAM auth tokens over TLS.
"$MYSQL_BIN" -h 127.0.0.1 -P "$LOCAL_PORT" -u "$DB_USER" \
  --password="$TOKEN" \
  --enable-cleartext-plugin \
  --ssl-mode=VERIFY_CA --ssl-ca=/etc/ssl/cert.pem \
